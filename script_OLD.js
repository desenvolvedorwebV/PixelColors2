...
const paletteHandle = document.getElementById("paletteHandle");
...
paletteHandle.addEventListener("click", () => togglePalette());
paletteHandle.addEventListener("pointerdown", (event) => {
    paletteDragStart = event.clientY;
    paletteHandle.setPointerCapture(event.pointerId);
});
paletteHandle.addEventListener("pointermove", (event) => {
    if (paletteDragStart === null) {
        return;
    }

    const delta = event.clientY - paletteDragStart;
    if (delta < -30) {
        togglePalette(true);
        paletteDragStart = null;
        paletteHandle.releasePointerCapture(event.pointerId);
    } else if (delta > 30) {
        togglePalette(false);
        paletteDragStart = null;
        paletteHandle.releasePointerCapture(event.pointerId);
...
paletteHandle.addEventListener("pointerup", (event) => {
    paletteDragStart = null;
    paletteHandle.releasePointerCapture(event.pointerId);
});
paletteHandle.addEventListener("pointercancel", (event) => {
    paletteDragStart = null;
    paletteHandle.releasePointerCapture(event.pointerId);
});

function togglePalette(forceOpen = null) {
    if (forceOpen === null) {
        paletteOpen = !paletteOpen;
    } else {
        paletteOpen = forceOpen;
    }

    setPaletteOpenState(paletteOpen);
}

function setPaletteOpenState(open) {
    paletteOpen = open;
    if (open) {
        palette.classList.add("open");
        paletteHandle.textContent = "Fechar paleta";
    } else {
        palette.classList.remove("open");
        paletteHandle.textContent = "Paleta de cores";
    }
}
...
function salvarProgressoAtual(completo = false) {
    ...
    localStorage.setItem(
        "pixelColorProgress",
        JSON.stringify({ percent: progressoPorNivel, painted: pintadasPorNivel })
    );
}
